...
public class ContourRectum {
	...
	public static Object[] run(XMimSession sess, XMimImage img) {
		...
		// Path to the model
		String modelRectumPath = "D:\\MIMPelvisAutoContouring\\sampleWorkspace\\autocontouring\\resources\\model_rectum_11_16";
		...
		try {
			if (foundFemrRtContour == true) {
				if (foundBodyContour == true) {
					...
					// write to log file (no Body contour found)
					String errorB = "Body contour not found";

				}
			} else {
				// write to log file (no FemrRt contour found)
				String errorFR = "FemrRt contour not found";
